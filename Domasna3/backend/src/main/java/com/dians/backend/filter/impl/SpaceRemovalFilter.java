...
public class SpaceRemovalFilter implements Filter<String> {
    @Override
    public String execute(String input) {
        ...
        if (fourthColumnIndex >= 0 && fourthColumnIndex < columns.length) {
            // Remove spaces from the 4th column
            columns[fourthColumnIndex] = columns[fourthColumnIndex].replaceAll("\\s", "");
        }
